...
module tb_rxuartlite;
    ...
    task run_case;
        ...
        input [7:0] data;
        begin
            // Reset the previous-state tracker at the beginning of each case
            // so that the per-case DUT path starts from IDLE.
            prev_state = dut.state;

            case (case_no)
                1: $display("[%0t] TB_PATH: CASE1 normal receive start", $time);
                2: $display("[%0t] TB_PATH: CASE2 normal receive start", $time);
                3: $display("[%0t] TB_PATH: CASE3 normal receive start", $time);
                4: $display("[%0t] TB_PATH: CASE4 normal receive start", $time);
                default: $display("[%0t] TB_PATH: CASE%0d normal receive start", $time, case_no);
            endcase
    ...
    initial begin
        ...
        pass_count      = 0;
        fail_count      = 0;
        path_log_enable = 0;             // suppress UNKNOWN -> IDLE before CASE1
        prev_state      = 4'hx;
        ...
        // Enable DUT path logging for all normal receive cases.
        path_log_enable = 1;

        // Basic normal-operation patterns:
        // CASE1 0x00: all zero data bits
        // CASE2 0xFF: all one data bits
        // CASE3 0x55: alternating pattern starting with bit0=1
        // CASE4 0xAA: alternating pattern starting with bit0=0
        run_case(1, 8'h00);
        run_case(2, 8'hff);
        run_case(3, 8'h55);
        run_case(4, 8'haa);
