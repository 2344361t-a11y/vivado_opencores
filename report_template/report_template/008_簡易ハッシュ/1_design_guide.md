...
- `text_o[31:0]`: 計算結果のSHA-256ハッシュ値を32 bitずつ出力する信号
...
- `read_counter[2:0]`: 256 bitハッシュ値を8個の32 bit wordとして読み出すためのカウンタ
...
- `cmd[0]=1`の読み出しコマンドを受けると、`read_counter`を7に設定し、`busy=0`の間に`text_o`へ上位wordから順番にハッシュ値を出力する。
...
- この確認は、基本動作確認である`CASE1`および各ブロック処理の中に含める。

#### `text_o[31:0]`によるハッシュ値読み出し確認

- 読み出しコマンド`cmd_i=3'b001`の後、`text_o`から32 bit wordが8回出力されることを確認する。
- 読み出し順序は、SHA-256ハッシュ値の上位32 bitから下位32 bitの順である。
- 各wordを期待値と比較し、最後に256 bitのハッシュ値全体が一致することを確認する。

#### 複数ブロック処理の確認

- 1ブロック目では`cmd_i=3'b010`を用い、SHA-256初期値から計算する。
- 2ブロック目では`cmd_i=3'b110`を用い、1ブロック目の計算結果を引き継いで計算する。
- 複数ブロック入力の最終ハッシュ値が既知値と一致することで、内部状態の引き継ぎが正しいことを確認する。
...
- 入力メッセージをSHA-256の512 bitブロックへ変換した状態で与え、回路のブロック処理およびハッシュ値読み出しを検証する。
- テスト実行経路、入力word、内部状態、判定結果をシミュレーションログとして出力する。

### テストケース

- `RESET`: リセット中に`cmd_o=4'b0000`、`text_o=32'h00000000`、内部`busy=0`であることを確認する。
- `CASE1`: 文字列`abc`を1ブロックのSHA-256入力として処理し、既知のハッシュ値と一致することを確認する。
- `CASE2`: 空入力をpaddingのみの1ブロックとして処理し、既知のハッシュ値と一致することを確認する。
- `CASE3`: OpenCores付属テストベンチで用いられている長い文字列を2ブロックで処理し、継続ブロック処理が正しいことを確認する。
