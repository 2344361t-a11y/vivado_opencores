...
## 構成図（ブロック図）
![FIFO非同期回路図](./images/fifo_dc.png)

## 回路図
![FIFO同期回路図](./images/fifo_dc_module.png)
...
- `CASE1`
  - 正常受信後に `data_valid=1` になることを確認する。
- `CASE1`
  - `8'h11`, `8'h22`, `8'h33`, `8'h44` を書き込み、同じ順番で読み出されることを確認する。
- `CASE2`
  - `8'h00` から `8'hFF` までのデータを順番に書き込み、先頭データ `8'h00`、残りの連続データ、追加データ `8'hA5` が正しく読み出されることを確認する。
...
本回路では、`generic_fifo_sc_a.v` および `generic_dpram.v` の先頭で以下のように読み込まれる。
`include "timescale.v"
