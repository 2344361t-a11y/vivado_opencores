...
| 項目 | 入力条件 | 期待値 | 実測値 | 判定 |
| --- | --- | --- | --- | --- |
| `full` 到達 | 256個書き込み後 | `full=1` | `full=1` | 合格 |
| `empty` | 256個書き込み後 | `empty=0` | `empty=0` | 合格 |
| ポインタ | 256個書き込み後 | `wp==rp` | `wp=0x04`, `rp=0x04` | 合格 |
| guard bit | 256個書き込み後 | `gb=1` | `gb=1` | 合格 |
| データ数 | 256個書き込み後 | `cnt=256` | `cnt=256` | 合格 |
| 読み出し順序 | 256個読み出し | 書き込み順と一致 | 全256個一致 | 合格 |
| `empty` 復帰 | 全データ読み出し後 | `empty=1` | `empty=1` | 合格 |
| `full` | 全データ読み出し後 | `full=0` | `full=0` | 合格 |
| ポインタ | 全データ読み出し後 | `wp==rp` | `wp=0x04`, `rp=0x04` | 合格 |
| guard bit | 全データ読み出し後 | `gb=0` | `gb=0` | 合格 |
| データ数 | 全データ読み出し後 | `cnt=0` | `cnt=0` | 合格 |

### CASE3 `clr` によるクリア確認
| 項目 | 入力条件 | 期待値 | 実測値 | 判定 |
| --- | --- | --- | --- | --- |
| クリア前状態 | `8'h00`, `8'hFF`, `8'hA5`, `8'h5A` を書き込み | `empty=0`, `cnt=4` | `empty=0`, `cnt=4` | 合格 |
| `clr` 入力後 | `clr=1` を入力 | `wp=0`, `rp=0`, `gb=0`, `cnt=0`, `full=0`, `empty=1` | `wp=0`, `rp=0`, `gb=0`, `cnt=0`, `full=0`, `empty=1` | 合格 |
