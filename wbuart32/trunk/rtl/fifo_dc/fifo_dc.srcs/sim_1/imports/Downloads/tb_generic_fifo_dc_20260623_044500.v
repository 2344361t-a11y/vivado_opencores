...
module tb_generic_fifo_dc;
    ...
    initial begin
        ...
        $display("[%0t] TB_CASE: write 00,FF,A5,5A then read in same order", $time);
        write_one(8'h00);
        write_one(8'hFF);
        write_one(8'hA5);
        write_one(8'h5A);
        wait_empty_value(1'b0, "CASE1 empty must become 0 after writes");
        read_check(8'h00, "CASE1 read data must be 0x00");
        read_check(8'hFF, "CASE1 read data must be 0xFF");
        read_check(8'hA5, "CASE1 read data must be 0xA5");
        read_check(8'h5A, "CASE1 read data must be 0x5A");
        ...
        $display("[%0t] TB_CASE: write 00-FF then read 00-FF", $time);
        ...
        if (bulk_error_count == 0) begin
            tb_pass("CASE2 remaining 0x01-0xFE data must match");
        end else begin
            tb_fail("CASE2 remaining 0x01-0xFE data must match");
        end

        read_check(8'hFF, "CASE2 last read data must be 0xFF");
        ...
        $display("[%0t] TB_CASE: write 00,FF,A5,5A then clear", $time);
        write_one(8'h00);
        write_one(8'hFF);
        write_one(8'hA5);
        write_one(8'h5A);
