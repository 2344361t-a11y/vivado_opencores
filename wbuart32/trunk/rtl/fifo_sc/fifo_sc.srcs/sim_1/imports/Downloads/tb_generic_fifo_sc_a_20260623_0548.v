...
// Revision   : CASE3 uses the same basic input pattern as CASE1 (00, FF, A5, 5A)

module tb_generic_fifo_sc_a_20260623_0548;
    ...
    initial begin
        ...
        // Use the same basic input pattern as CASE1 for consistency.
        $display("[%0t] TB_PATH: CASE3 clr behavior start", $time);
        $display("[%0t] TB_INFO: CASE3 pre-clear write sequence = 00, FF, A5, 5A", $time);
        for (i = 0; i < 4; i = i + 1) begin
            write_one(case1_data[i], "CASE3 pre-clear");
        end
        check_bit(empty === 1'b0, "CASE3 empty must be 0 before clr");
        check_eq_int(dut.cnt, 4, "CASE3 cnt must be 4 before clr");
