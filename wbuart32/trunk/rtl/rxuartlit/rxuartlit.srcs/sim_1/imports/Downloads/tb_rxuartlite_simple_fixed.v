...
module tb_rxuartlite_simple_fixed;
    ...
    //
    // Important:
    // The previous version waited for one negedge before changing the signal,
    // and then waited CLOCKS_PER_BAUD more negedges. When tasks were called
    // back-to-back, each bit lasted CLOCKS_PER_BAUD + 1 clocks.
    //
    // This version keeps the interval between two bit changes exactly
    // CLOCKS_PER_BAUD clocks.
    task drive_bit_for_one_baud;
        ...
        begin
            ...
            // k starts at 1 because the next task call also waits for one negedge
            // before changing to the next bit. Total interval = CLOCKS_PER_BAUD.
    ...
    initial begin
        $dumpfile("tb_rxuartlite_simple_fixed.vcd");
        $dumpvars(0, tb_rxuartlite_simple_fixed);
        ...
        // Error-like case 3: line held low for a long time.
        // There is no error output, but a valid byte should not be reported.
        before_count = wr_count;
        @(negedge i_clk);
        i_uart_rx = 1'b0;
