...
module tb_rxuartlite_normal;
    ...
    task expect_receive;
        ...
        begin
            ...
            check(got_wr == 1, "o_wr must assert after a valid 8N1 frame");

            if (got_wr == 1) begin
                $display("[%0t] TB_INFO: o_wr=1 o_data=0x%02h expected=0x%02h",
                         $time, o_data, expected);
                check(o_data === expected, "o_data must match transmitted byte");

                @(posedge i_clk);
                #1;
                check(o_wr == 1'b0, "o_wr must be a one-clock pulse");
            end
        end
    endtask
    ...
    task run_case;
        ...
        begin
            fork
                ...
                expect_receive(data);
            join
            repeat (3 * BAUD) @(posedge i_clk);
        end
    endtask

    initial begin
        i_uart_rx       = 1'b1;          // UART idle level
        pass_count      = 0;
        fail_count      = 0;
        path_log_enable = 1;
        prev_state      = 4'hx;
        ...
        repeat (4 * BAUD) @(posedge i_clk);
        check(o_wr == 1'b0, "o_wr must stay 0 during idle before reception");
